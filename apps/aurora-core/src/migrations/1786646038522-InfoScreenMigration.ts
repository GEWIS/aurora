...
export class InfoScreenMigration1786646038522 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`pc_status\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`pcId\` varchar(255) NOT NULL, \`users\` text NULL, \`remote\` tinyint NOT NULL DEFAULT 0, \`lockedAt\` datetime NULL, \`status\` varchar(255) NOT NULL DEFAULT 'offline', UNIQUE INDEX \`IDX_bae498a3affa96972742f4c207\` (\`pcId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`keyholder\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`name\` varchar(255) NOT NULL, \`displayName\` varchar(255) NULL, \`isBoard\` tinyint NOT NULL DEFAULT 0, \`isCandidateBoard\` tinyint NOT NULL DEFAULT 0, \`isKeyholder\` tinyint NOT NULL DEFAULT 0, \`photoUrl\` varchar(255) NULL, \`memberId\` int NULL, UNIQUE INDEX \`IDX_d77b1cc8fe4a283a33549c7da1\` (\`memberId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`room_status\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`open\` tinyint NOT NULL DEFAULT 0, \`responsible1MemberId\` int NULL, \`responsible2MemberId\` int NULL, \`beerTime\` varchar(255) NULL, \`lastCall\` varchar(255) NULL, \`closedMessage\` varchar(255) NULL, \`coffeeStatus\` int NOT NULL DEFAULT '0', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
