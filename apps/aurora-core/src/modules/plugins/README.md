...
scope.registerInstance(ModeSession, session);
...
`scope.resolve` reads the constructor, sees it wants a `LightsControl`, and hands over what
...
`registerPort` in `src/ioc.ts` exists because tsyringe's types expect a concrete class; it
