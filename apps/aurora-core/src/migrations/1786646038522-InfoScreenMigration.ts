...
export class InfoScreenMigration1786646038522 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`pc_status\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`pcId\` varchar(255) NOT NULL, \`username\` varchar(255) NULL, \`remote\` tinyint NOT NULL DEFAULT 0, \`lockedAt\` datetime NULL, \`status\` varchar(255) NOT NULL DEFAULT 'offline', \`overrideState\` varchar(255) NOT NULL DEFAULT 'none', UNIQUE INDEX \`IDX_bae498a3affa96972742f4c207\` (\`pcId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`keyholder\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`name\` varchar(255) NOT NULL, \`isBoard\` tinyint NOT NULL DEFAULT 0, \`isCandidateBoard\` tinyint NOT NULL DEFAULT 0, \`isKeyholder\` tinyint NOT NULL DEFAULT 0, \`photoUrl\` varchar(255) NULL, \`usernames\` text NULL, \`ldapGuid\` varchar(32) NULL, \`ldapName\` varchar(255) NULL, UNIQUE INDEX \`IDX_6ae9ab72d7701a981d974b16bc\` (\`ldapGuid\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`room_status\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`open\` tinyint NOT NULL DEFAULT 0, \`responsible1\` varchar(255) NULL, \`responsible2\` varchar(255) NULL, \`beerTime\` varchar(255) NULL, \`lastCall\` varchar(255) NULL, \`lastResetAt\` datetime NULL, \`closedMessage\` varchar(255) NULL, \`coffeeStatus\` int NOT NULL DEFAULT '0', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`DROP INDEX \`IDX_6ae9ab72d7701a981d974b16bc\` ON \`keyholder\``);
