...
export async function createApiKey(): Promise<string> {
  ...
  await getDataSource().getRepository(apiKey).save({ key });
  return key;
}

export async function createIntegrationKey(endpoints: string[]): Promise<string> {
  const user = await getDataSource().getRepository(integrationUser).save({
    name: 'Test Integration',
    endpoints,
  });

  const key = 'test-int-' + crypto.randomBytes(8).toString('hex');
  await getDataSource().getRepository(apiKey).save({
    key,
