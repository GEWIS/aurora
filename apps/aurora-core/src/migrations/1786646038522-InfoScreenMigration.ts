...
export class InfoScreenMigration1786646038522 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`pc_status\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`pcId\` varchar(255) NOT NULL, \`usernames\` text NULL, \`remote\` tinyint NOT NULL DEFAULT 0, \`lockedAt\` datetime NULL, \`status\` varchar(255) NOT NULL DEFAULT 'offline', \`overrideState\` varchar(255) NOT NULL DEFAULT 'none', UNIQUE INDEX \`IDX_bae498a3affa96972742f4c207\` (\`pcId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
